  ...
  ***************************************************************************** */
        ...
        if (isArray(target)) {
          warn$2(
            'Avoid using Array as root value for '
              .concat(shallow ? 'shallowReactive()' : 'reactive()', ' as it cannot be tracked in watch() or watchEffect(). Use ')
      ...
      Object.keys(result).forEach(function (key) {
        /* istanbul ignore else */
        {
          defineReactive(vm, key, result[key], function () {
            warn$2(
              'Avoid mutating an injected value directly since the changes will be ' + 'overwritten whenever the provided component re-renders. ' + 'injection being mutated: "'.concat(key, '"'),
              vm
            );
          });
        }
          ...
          tree
            .map(function (vm, i) {
              return ''
                ...
                .concat(isArray(vm) ? ''.concat(formatComponentName(vm[0]), '... (').concat(vm[1], ' recursive calls)') : formatComponentName(vm));
            })
            .join('\n')
        );
      } else {
  ...
  function genCheckboxModel(el, value, modifiers) {
    ...
    addProp(
      ...
      'Array.isArray('.concat(value, ')') +
        ...
        (trueValueBinding === 'true' ? ':('.concat(value, ')') : ':_q('.concat(value, ',').concat(trueValueBinding, ')'))
    );
    addHandler(
      el,
      'change',
  ...
  function processIfConditions(el, parent) {
    ...
    if (prev && prev.if) {
      ...
      warn(
        'v-'.concat(el.elseif ? 'else-if="' + el.elseif + '"' : 'else', ' ') + 'used on element <'.concat(el.tag, '> without corresponding v-if.'),
  ...
  function processSlotContent(el) {
    ...
    if (el.tag === 'template') {
      ...
      if (el.attrsMap['v-for']) {
        warn(
          'Ambiguous combined usage of slot-scope and v-for on <'.concat(el.tag, '> ') + '(v-for takes higher priority). Use a wrapper <template> for the ' + 'scoped slot to make it clearer.',
